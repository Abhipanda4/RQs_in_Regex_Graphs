...
vector< vector<float> > compute_min_label(
    ...
    auto t2 = std::chrono::duration_cast<std::chrono::milliseconds>(diff);
...
vector< pair<int, char> > monte_carlo(
    ...
    diff = std::chrono::high_resolution_clock::now() - start;
    auto t3 = std::chrono::duration_cast<std::chrono::milliseconds>(diff);
    ...
    diff = std::chrono::high_resolution_clock::now() - start;
    auto t4 = std::chrono::duration_cast<std::chrono::milliseconds>(diff);

    cout << t1.count() << " -- " << t2.count() << " -- " << t3.count() << " -- " << t4.count() << endl;
    return result;
}
...
int main(int argc, char* argv[]) {
    for (int MC_nodes = 1; MC_nodes <= 5; MC_nodes++) {
        ...
        while (querysize--) {
            ...
            num_queries += 1;
            cin >> uatt >> vatt >> regex;
            cout << versize << ", " << t1.count() << ", " << uatt << ", " << vatt << ", " << regex << ", " << endl;
            start = std::chrono::high_resolution_clock::now();
